...
const ReservationPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchShowtimes = async () => {
            try {
                const response = await fetch(`http://localhost:3000/showtimes/get-all-showtimes?movie_id=${movie_id}&theater_id=${selectedTheater}&date=${selectedDate}`);
                if (!response.ok) {
                    throw new Error('Failed to fetch showtimes');
                }

                const data: Showtime[] = await response.json();
                const uniqueTheaters = [...new Set(data.map((s) => s.theater_name))];
                const uniqueDates = [
    ...
    }, [movie_id, selectedTheater, selectedDate]);
